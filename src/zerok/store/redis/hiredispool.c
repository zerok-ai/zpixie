...
{
    printf("%s\n", "zk-log/stores DEBUG01 redis_pool_create");
    int i;
    char* host;
    int port;
    REDIS_INSTANCE *inst;
    ...
    printf("%s\n", "zk-log/stores DEBUG02 redis_pool_create ");

    inst->config = malloc(sizeof(REDIS_CONFIG));
    memset(inst->config, 0, sizeof(REDIS_CONFIG));

    printf("%s\n", "zk-log/stores DEBUG03 redis_pool_create ");
    ...
    printf("%s\n", "zk-log/stores DEBUG04 redis_pool_create ");
    ...
    printf("%s\n", "zk-log/stores DEBUG05 redis_pool_create ");
    ...
    printf("%s\n", "zk-log/stores DEBUG06 redis_pool_create ");
    ...
    for (i = 0; i < inst->config->num_endpoints; i++) {
        ...
        printf("%s %s\n", "zk-log/stores DEBUG07 redis_pool_create ", host);
    }
    ...
    if (redis_init_socketpool(inst) < 0) {
        printf("%s\n", "zk-log/stores DEBUG08 redis_pool_create ");
        redis_pool_destroy(inst);
        return -1;
    }
...
static int redis_init_socketpool(REDIS_INSTANCE * inst)
{
    ...
    for (i = 0; i < inst->config->num_redis_socks; i++) {
        ...
        printf("%s\n", "zk-log/stores DEBUG01 redis_init_socketpool ");
        ...
        printf("%s\n", "zk-log/stores DEBUG02 redis_init_socketpool ");
        ...
        if (time(NULL) > inst->connect_after) {
            ...
            if (connect_single_socket(redisocket, inst) == 0) {
                printf("%s\n", "zk-log/stores DEBUG03 redis_init_socketpool ");
                success = 1;
            }
        }
