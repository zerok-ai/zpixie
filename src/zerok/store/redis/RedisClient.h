...
class PooledSocket {
 ...
 public:
  ...
  PooledSocket(REDIS_INSTANCE* _inst) : inst(_inst) { sock = redis_get_socket(inst); }
  // Release the socket to pool
  ~PooledSocket() { redis_release_socket(inst, sock); }
...
class RedisReplyPtr {
 public:
  ...
  // transfer ownership
  RedisReplyPtr(RedisReplyPtr& other) { p = other.release(); }
  ...
  RedisReplyPtr(RedisReplyRef _ref) { p = _ref.p; }
  RedisReplyPtr& operator=(RedisReplyRef _ref) {
    if (p == _ref.p) return *this;
    RedisReplyPtr temp(release());
    p = _ref.p;
    return *this;
  }
...
class RedisClient {
 ...
 public:
  ...
  ~RedisClient() { redis_pool_destroy(inst); }
